...
function MemberRow({ member, isFirst }: { member: Member; isFirst: boolean }) {
  const [imgError, setImgError] = useState(false);
  const initials = member.name.split(/\s+/).map((n) => n[0]).join("").slice(0, 2).toUpperCase() || "?";
  const showPlaceholder = !member.imageUrl || imgError;
  return (
    <div
      className={`flex items-center gap-4 p-4 ${!isFirst ? "border-t border-[#E2E0DB]" : ""}`}
    >
      <div className="w-10 h-10 rounded-full bg-[#1D2A3F] shrink-0 overflow-hidden flex items-center justify-center">
        {showPlaceholder ? (
      ...
      <div className="flex-1 min-w-0">
        <p className="font-inter font-semibold text-[#1D2A3F]">{member.name}</p>
        ...
        <div className="bg-white rounded-xl border border-[#E2E0DB] overflow-hidden">
          {filtered.length === 0 ? (
            ...
            filtered.map((member, i) => (
              <MemberRow key={member.id} member={member} isFirst={i === 0} />
            ))
          )}
        </div>
